...
def take_long_order(mode_take_long_order,*args,**kwargs):
    ...
    if float(spot_size) != 0:
        try:
            A = client.create_order(symbol=spot_instrument_id, side='BUY',type=spot_order_type, price=spot_price, quantity=spot_size)
        except:
            A = 'none'
    else:
        A='none'
...
def take_short_order(mode_take_short_order,*args,**kwargs):
    ...
    if float(spot_size) != 0:
        try:
            A = client.create_order(symbol=spot_instrument_id,side='SELL',type=spot_order_type,price=spot_price, quantity=spot_size)
        except:
            A = 'none'
    else:
        A='none'
...
def take_close_long_order(mode_take_close_long_order,*args,**kwargs):
    ...
    if float(spot_close_size) != 0:
        try:
            A = client.create_order(symbol=spot_instrument_id, side='SELL',type=spot_order_type, price=spot_close_price, quantity=spot_close_size)
        except:
            A='none'
    else:
        A = 'none'
...
def take_close_short_order(mode_take_close_short_order,*args,**kwargs):
    ...
    if float(spot_close_size) != 0:
        try:
            A = client.create_order(symbol=spot_instrument_id, side='BUY',type=spot_order_type, price=spot_close_price, quantity=spot_close_size)
        except:
            A = 'none'
    else:
        A = 'none'
...
def close_long_judge(mode_close_long_judge, *args, **kwargs):
    ...
    if spot_balance - Target_Amount_Close > 0 and abs(spot_balance - Target_Amount_Close) > float(spot_close_size):  # spot超过TA,要平仓
        ...
        if -1*swap_position - spot_balance > float(swap_min_size):  # swap多于spot,平空
            close_long_final_close_mode = 'on'
        elif spot_balance + swap_position > float(swap_min_size):  # swap少于spot,开空
...
def close_short_judge(mode_close_short_judge, *args, **kwargs):
    ...
    if spot_balance - Target_Amount_Close < 0 and abs(spot_balance - Target_Amount_Close) > float(spot_close_size):  # spot超过TA,要平仓
        ...
        #swap仓位比spot多
        if (swap_position + spot_balance )>0 and abs(swap_position + spot_balance) > float(swap_min_size):  # swap多于spot,平多
            close_short_final_close_mode = 'on'
        #spot仓位比swap多
        elif (spot_balance + swap_position )<0 and abs(spot_balance + swap_position) > float(swap_min_size):  # swap少于spot,开多
...
def open_long_judge(mode_open_long_judge, *args, **kwargs):
    ...
    if Target_Amount - spot_balance > 0 and abs(Target_Amount - spot_balance) > float(spot_size):  # 还大于一个 spot_size,继续建仓
        ...
        if spot_balance + swap_position > float(swap_min_size):       #因为posSide是net,所以持仓本身带著正负
            open_long_final_open_mode = 'on'  # swap要建仓 开空
        elif -1*swap_position - spot_balance > float(swap_min_size):
...
def open_short_judge(mode_open_short_judge, *args, **kwargs):
    ...
    if Target_Amount - spot_balance < 0 and abs(Target_Amount - spot_balance) > float(spot_size):  # 还大于一个spot_size,继续建仓
        ...
        if  spot_balance + swap_position <0 and abs(spot_balance + swap_position) > float(swap_min_size):
            open_short_final_open_mode = 'on'
        #swap建仓比spot多,swap要平仓 平多
        elif spot_balance + swap_position >0 and abs(swap_position+ spot_balance) > float(swap_min_size):
...
async def MARGIN_Account_update(api_key, api_secret,AQ,TradingPair):
    while True:
        try:
            client = Client(api_key, api_secret)
            #client = await AsyncClient.create()
            ...
            async with ts as tscm:
                ...
                #        print(res)
                    ...
                    if res['e'] =='outboundAccountPosition':
                        res_spot_ccy_list = []
                        for i in TradingPair:           #这里从TP中做出做res_spot_ccy_list
                            TP=i.split(i[-4:])[0]
                            res_spot_ccy_list.append(TP)

                        stream_spot_list=[]
                        for i in res['B']:              #从送来的res中做出spot_list
                            stream_spot_list.append(i['a'])                            
                        ...
                        for j in res_spot_ccy_list:
                            if j in stream_spot_list:
                                ...
                                for k in res['B']:
                                    if k['a']==j:
                                        TP=k['a']+'USDT'  #按照全体格式,这里再加usdt
                                        if AQ[TP]["POSITION_SPOT"].empty() == True:
                                            AQ[TP]["POSITION_SPOT"].put(k['f'])
                                        elif AQ[TP]["POSITION_SPOT"].empty() == False:
                                            AQ[TP]["POSITION_SPOT"].get()
                                            AQ[TP]["POSITION_SPOT"].put(k['f'])
                            else:
    ...
    while True:   
        try:
            ...
            while True:
                ...
                #          print(Operation_info[h]['spot_balance'])
                        # 找swap_position
                        try:
                            ...
                            for i in total_swap_position_result['positions']:
                                if i['symbol']==h:
                                    Operation_info[h]['swap_position']=float(i['positionAmt'])  #注意这里是持仓数目,不是notional
            #              print(Operation_info[h]['swap_position'])
                        ...
                        if no_pending_order == 'on': #先看有没有遗留单,有就不走调仓判断
                            if Operation_info[h]['spot_balance'] > 0 : #这时候是spot开多,swap开空
                                if Operation_info[h]['spot_balance'] * new_spot_present_price > Necessary_info[h]['Total_money'] * 1.1:
                                    So_Sc_mode = 'off'
...
#                                                        'spot_size': spot_size,
    ...
    #                              open_long_mode, open_long_final_open_mode, open_long_final_close_mode = open_long_judge(mode_open_long_judge)
            ...
            #                        open_short_mode, open_short_final_open_mode, open_short_final_close_mode = open_short_judge(mode_open_short_judge)
                        ...
                        if no_pending_order == 'on':  # 先看有没有遗留单,有就不走持仓差调整
                            if Operation_info[h]['position_direction']=='open':   #现在是开仓方向
                                if Operation_info[h]['spot_balance'] > 0: #现在是开正仓
                                    if abs(Operation_info[h]['spot_balance'] + Operation_info[h]['swap_position']) * new_spot_present_price  > (tolerate_limit*1.5 ):
                                        So_Sc_mode = 'off'
                                        mode_open_long_judge = {'Target_Amount': Target_Amount,
                                                                    'spot_balance': Operation_info[h]['spot_balance'],
                                                                    'swap_position': Operation_info[h]['swap_position'],
                                ...
                                elif Operation_info[h]['spot_balance'] < 0: #现在是开负仓,但不是margin模式不会遇到,先写著
                                    if abs(Operation_info[h]['spot_balance'] + Operation_info[h]['swap_position']) * new_spot_present_price  > (tolerate_limit*1.5):
                                        So_Sc_mode = 'off'
                                        #查询开负仓还有多少可开
                                        max_avail_size_result = float(client.get_max_margin_loan(asset=h.split(h[-4:])[0],isolatedSymbol=h)['amount'])
                                        ...
                                        if max_avail_size_result<spot_size:
                                            So_Sc_mode = 'off'
                                            if abs(Operation_info[h]['swap_position']) > abs(Operation_info[h]['spot_balance']):
                        ...
                        #                        open_short_mode, open_short_final_open_mode, open_short_final_close_mode = open_short_judge(mode_open_short_judge)
                                                close_short_mode, close_short_final_open_mode, close_short_final_close_mode = close_short_judge(mode_close_short_judge)
                                            elif abs(Operation_info[h]['swap_position']) < abs(Operation_info[h]['spot_balance']):
                                                # 已经没法继续开负仓,然后v现货约多于合约仓位,TA就等于目前的spot_balance然后不要增加,建swap的仓位
                                                Target_Amount= abs(Operation_info[h]['spot_balance'])  #因为在def会*-1,所以这里加上abs                                                
        ...
        #                              open_long_mode, open_long_final_open_mode, open_long_final_close_mode = open_long_judge(mode_open_long_judge)
                ...
                #         print('swap_ask',Operation_info[h]['swap_asks_price5'])
                        ...
                        if open_long_mode == 'on':
                            if (tolerate_limit/2/swap_present_price) > Operation_info[h]['spot_balance']+ Operation_info[h]['swap_position'] > \
                            ...
                            elif (tolerate_limit / swap_present_price) > Operation_info[h]['spot_balance']+ Operation_info[h]['swap_position'] > \
                            ...
                            elif Operation_info[h]['spot_balance'] + Operation_info[h]['swap_position'] > (tolerate_limit / swap_present_price):
                            ...
                            elif -1*(tolerate_limit / 2 /  swap_present_price) < Operation_info[h]['swap_position'] + Operation_info[h]['spot_balance'] <-1*(tolerate_limit / 4 / swap_present_price):  
                            ...
                            elif -1*(tolerate_limit / swap_present_price) < Operation_info[h]['swap_position'] + Operation_info[h]['spot_balance'] <-1*(tolerate_limit / 2 / swap_present_price):  
                        ...
                        if open_short_mode == 'on':  #swap是long_position,spot_balance<0
                            if -1*(tolerate_limit / 2 / swap_present_price) < Operation_info[h]['spot_balance'] + Operation_info[h]['swap_position'] < -1*(tolerate_limit / 4 / swap_present_price):
                            ...
                            elif -1*(tolerate_limit / swap_present_price) < Operation_info[h]['spot_balance'] + Operation_info[h]['swap_position'] < -1*(tolerate_limit / 2 / swap_present_price):
                            ...
                            elif Operation_info[h]['spot_balance'] + Operation_info[h]['swap_position'] < -1*(tolerate_limit / swap_present_price):
                            ...
                            elif (tolerate_limit / 2 / swap_present_price) > Operation_info[h]['swap_position'] + Operation_info[h]['spot_balance'] >(tolerate_limit / 4 / swap_present_price):
                            ...
                            elif (tolerate_limit / swap_present_price) > Operation_info[h]['swap_position'] + Operation_info[h]['spot_balance'] >(tolerate_limit / 2 / swap_present_price):
                            ...
                            elif Operation_info[h]['swap_position'] + Operation_info[h]['spot_balance'] > (tolerate_limit / swap_present_price):
                        ...
                        if close_short_mode =='on':  #平负仓，spot平空,swap平多.spot_balance<0,swap_position>0
                            if -1*(tolerate_limit / 2 / swap_present_price) < Operation_info[h]['spot_balance'] + Operation_info[h]['swap_position'] < -1*(tolerate_limit / 4 / swap_present_price):  
                                ...
                                #平负仓，spot平空买入,swap平多卖出.spot_balance<0,swap_position>0
                            ...
                            elif (tolerate_limit / swap_present_price) > Operation_info[h]['swap_position'] + Operation_info[h]['spot_balance'] > (tolerate_limit / 2 / swap_present_price):  #平负仓，spot平空买入,swap平多卖出.spot_balance<0,swap_position>0
                                ...
                                #平负仓，spot平空,swap平多.spot_balance<0,swap_position>0
                        ...
                        if close_long_mode =='on':  
                            if (tolerate_limit / 4 / swap_present_price) > Operation_info[h]['spot_balance'] + Operation_info[h]['swap_position'] > (tolerate_limit / 4 / swap_present_price):
                            ...
                            elif (tolerate_limit / swap_present_price) > Operation_info[h]['spot_balance'] + Operation_info[h]['swap_position'] > (tolerate_limit / 2 / swap_present_price):
                                #spot平多,swap平空,spot_balance>0,swap_position<0
...
def run_ws(api_key, api_secret,TT,MQ,AQ,streams,TradingPair,param_set_list):
 ...
 #   pd.set_option('display.expand_frame_repr', False)  # 设置不折叠数据
    ...
    # 币安帐户
    #api_key = 'xja9O11Ov95zTFkEqEDL5IjLCg6rZQTZewflRGEdDx1YaGtJ2CxaVdcYxa7ZKY9N'
    #api_secret = 'S4srKg3csOXTI14BwoHSdqUQiocGeGcbMsTIdTU70LC5z6gtbCX4udSyaTge4Lna'
    #client = Client(api_key, api_secret)
    ...
    # margin_account_data
    if TT=="Margin_Account_data":
        loop.run_until_complete(MARGIN_Account_update(api_key, api_secret, AQ,TradingPair))
    # 交易（下单，撤单，改单等）
    if TT== "trade":
...
if __name__ == "__main__":   
    ...
    #币币杠杆帐户用不到 所以关掉
   # ws_Margin_Account_data = Process( target=run_ws, args=(api_key, api_secret,"Margin_Account_data",MarketQ,AccountQ,stream_Margin_Account,TradingPair,param_set_list))
    ...
    while True:
        try:     
            ...
            for i in TradingPair:
              ...
              # print('i-23',i)
              #  print('spot_balance_1',Operation_info[i]['spot_balance'])
                # 找swap_position
                try:
               ...
               # print('spot_balance_2',Operation_info[i]['spot_balance'])
                compare_cal=Operation_info[i]['swap_position']+Operation_info[i]['spot_balance']   
                print('compare_cal',compare_cal*spot_present_price)
                #因为一正一负,所以这里用加的
                if Operation_info[i]['spot_balance'] >0:  #开正仓  
                    if compare_cal*spot_present_price>200:       
                        ...
                        sendmessage(tutu)
                        time.sleep(3) 
                elif Operation_info[i]['spot_balance'] <0:  #开负仓 
                    if compare_cal*spot_present_price>200:       
                        tutu=timestamp +','+i+'永续比现货多'+ str(precfloat(abs(compare_cal*spot_present_price),3))+'美金'
                        sendmessage(tutu)
                        time.sleep(3)    
